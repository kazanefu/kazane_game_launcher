...
async fn test_start_and_stop_process() {
    ...
    let (exe, args): (PathBuf, Vec<String>) = {
        (PathBuf::from("ping"), vec!["-n".to_string(), "4".to_string(), "127.0.0.1".to_string()])
    };
    #[cfg(not(windows))]
    let (exe, args): (PathBuf, Vec<String>) = {
        (PathBuf::from("sleep"), vec!["2".to_string()])
    };

    let id = "test-process";
    let info = mgr.start(id, exe.clone(), &args.iter().map(|s| s.to_string()).collect::<Vec<_>>()).await.expect("start");
    assert!(info.pid != 0);
