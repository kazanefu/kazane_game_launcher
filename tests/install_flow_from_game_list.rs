...
async fn test_install_flow_steps_5_to_7_for_all_games() {
    ...
    for g in gl.games {
        if let Some((owner, repo)) = parse_owner_repo(&g.repo) {
            let installed = install_from_repo(&provider, &owner, &repo, &games_dir, &game_data).await.expect("install");
            // verify entry in local/game_data.json
            let local_data: kazane_game_launcher::data::local::LocalGameData = file::read_json(&game_data).expect("read game_data");
            let found = local_data.installed.iter().any(|it| it.id == repo || it.name == installed.name || it.install_path == installed.install_path);
