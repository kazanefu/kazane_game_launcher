    ...
    P: RemoteProvider + Send + Sync,
{
    ...
    /// Install by game id (lookup repo from game_list.json). Progress/cancel handled internally as None.
    pub async fn install_game_by_id(&self, id: &str) -> Result<InstalledGame, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some((owner, repo)) = Self::parse_owner_repo(&entry.repo) {
            let installed = crate::installer::install::install_from_repo(&self.provider, &owner, &repo, &self.games_dir, &self.game_data_path, None, None).await?;
            Ok(installed)
        } else {
